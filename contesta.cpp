...
// {
// public:
//   bool operator()(p &t1, p &t2) if (t1.b > t2.b) { return true; }
//   return false;
// };
void fun()
{
  ll increment, streak, amount;
  cin >> increment >> streak >> amount;
  vl temp_arr1(streak + 1, 0);
  temp_arr1[0] = 1;
  if (amount <= streak)
  {
    cout << "NO" << endl;
  ...
  for (ll i = 1; i <= streak; i++)
  {
    ll req = ceil(1.0 * (value + 1) / (increment - 1));
    temp_arr1[i] = req;
    amount -= req;
    if (amount < (ll)0)
    {
